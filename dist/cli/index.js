...
import { preventionConfigSchema, triggerConfigSchema, validateConfig, } from "../shared/config-schema.js";
import logger from "../shared/logger.js";
import { checkInjectionStatus, injectFiles, formatCheckResult, formatInjectionResults, } from "../injection/index.js";
import { analyzeProject, formatAnalysis, evaluateRules, formatEvaluations, } from "../analysis/index.js";
import { generateEnvReport, printBanner, printCompactBanner, } from "../environment/index.js";
...
export class WatcherCLI {
    ...
    setupCLI() {
        this.program
            .name("watcher")
            .description(chalk.blue("File watcher service") +
    ...
    setupStartCommand() {
        this.program
            ...
            .option("--process-existing", "Process existing files at startup (emit FILE_ADDED for each)")
            .option("--process-existing-delay <ms>", "Delay between each existing file event (default: 10ms)", "10")
            ...
            try {
                ...
                console.log(chalk.gray("  Prevention: " +
    ...
    setupStatusCommand() {
        this.program
            ...
            if (status.metrics.totalProcessingTime > 0) {
                const avg = status.metrics.filesProcessed > 0
                    ? Math.round(status.metrics.totalProcessingTime /
            ...
            for (const [name, modStatus] of Object.entries(status.modules)) {
                const state = modStatus && typeof modStatus === "object" && "running" in modStatus
        ...
        this.program
            ...
            try {
                ...
                if (!fs.existsSync(options.file)) {
                    console.error(chalk.red("✖") + " File not found: " + chalk.bold(options.file));
                    process.exit(1);
                }
                await this.service.initialize();
                console.log(chalk.blue("▶") + " Testing with file: " + chalk.cyan(options.file));
                ...
                if (fs.existsSync(preventionPath)) {
                    ...
                    if (result.valid) {
                        ...
                        if (!options.validate) {
                            ...
                            const enabled = preventionConfig.rules?.filter((r) => r.enabled).length || 0;
                            console.log("    " +
                                chalk.gray("Enabled: ") +
                ...
                if (fs.existsSync(triggerPath)) {
                    ...
                    if (result.valid) {
                        ...
                        if (!options.validate) {
                            ...
                            const enabled = triggerConfig.rules?.filter((r) => r.enabled).length || 0;
                            console.log("    " +
                                chalk.gray("Enabled: ") +
                                chalk.green(String(enabled)));
                        }
    ...
    setupTestAllCommand() {
        this.program
            ...
            try {
                ...
                if (!testFile) {
                    const tmpDir = path.join(process.env.TEMP || process.env.TMP || "/tmp", "watcher-test");
                    await fs.ensureDir(tmpDir);
                    testFile = path.join(tmpDir, "test-pipeline.ts");
                    await fs.writeFile(testFile, 'const x: string = "hello";\nconsole.log(x);\n');
                    cleanupNeeded = true;
                    console.log(chalk.gray("  Created test file: ") + chalk.cyan(testFile));
                }
                else if (!fs.existsSync(testFile)) {
                    console.error(chalk.red("✖") + " File not found: " + chalk.bold(testFile));
                    process.exit(1);
                }
                ...
                if (preventionResult) {
                    ...
                    console.log("   " + chalk.yellow("⚠ Skipped (module not available)"));
                }
    ...
    setupScanCommand() {
        this.program
            .command("scan")
            .description("One-shot scan: detect issues, fix errors, inject consignment files")
            ...
            .option("--agents <agents>", "Comma-separated agent types to inject (claude,generic,copilot,cursor,windsurf)")
            .action(async (options) => {
            try {
                ...
                console.log(chalk.gray(`   ${analysis.language}, ${analysis.packageManager}, ${triggeredRules.length} rules triggered`));
                console.log(chalk.gray(`   Done in ${Date.now() - analysisStart}ms\n`));
                // Step 1: Injection check
                if (doInject || !doFix) {
                    console.log(chalk.blue("1.") + chalk.bold(" Checking consignment files..."));
                    ...
                    if (doInject && injResult.missingCount > 0) {
                        console.log(chalk.blue("2.") + chalk.bold(" Injecting consignment files..."));
                        ...
                        console.log(chalk.gray(`   Done in ${Date.now() - injectStart}ms\n`));
                        report.corrections = injectResult;
                        report.summary.injected = injectResult.filter((r) => r.action === "created" || r.action === "updated").length;
                    }
                }
                // Step 3: File scanning + prevention
                if (doFix) {
                    console.log(chalk.blue(doInject ? "3." : "2.") +
                    ...
                    console.log(chalk.gray(`   Scanned ${files.length} files in ${Date.now() - scanStart}ms`));
                    console.log(chalk.gray(`   Issues: `) +
                        (issuesFound > 0
                            ? chalk.red(String(issuesFound))
                            : chalk.green("0")));
                    if (doFix) {
                        console.log(chalk.gray(`   Fixes:  `) +
                            (fixesApplied > 0
                ...
                console.log(chalk.gray("  Files scanned:    ") +
                ...
                console.log(chalk.gray("  Issues found:     ") +
                    (report.summary.issuesFound > 0
                        ...
                        : chalk.green("0")));
                if (doFix) {
                    console.log(chalk.gray("  Fixes applied:    ") +
                        chalk.green(String(report.summary.fixesApplied)));
                }
                if (doInject) {
                    console.log(chalk.gray("  Files injected:   ") +
                        chalk.green(String(report.summary.injected)));
                }
                console.log(chalk.gray("  Rules triggered:  ") +
                    chalk.yellow(String(report.summary.rulesTriggered)));
                console.log();
                // Write report
                if (reportPath) {
                    await fs.writeJson(reportPath, report, { spaces: 2 });
                    console.log(chalk.gray("  Report written: ") + chalk.cyan(reportPath));
                    console.log();
                }
            }
            catch (error) {
    ...
    setupAnalyzeCommand() {
        this.program
            ...
            try {
                ...
                console.log(chalk.bold("Summary\n"));
                console.log(chalk.gray("  Enforced rules:  ") +
                    chalk.yellow(String(enforced.length)));
                console.log(chalk.gray("  Suggested rules: ") +
                    chalk.cyan(String(suggested.length)));
                console.log(chalk.gray("  Total triggered: ") +
    ...
    printMetrics() {
        ...
        if (metrics.filesProcessed > 0) {
            ...
            if (metrics.totalProcessingTime > 0) {
                const avg = Math.round(metrics.totalProcessingTime / metrics.filesProcessed);
                console.log("  " +
                    chalk.gray("Avg processing time:") +
                    " " +
                    chalk.cyan(`${avg}ms`));
        ...
        this.program
            ...
            try {
                ...
                for (const tool of report.missingTools) {
                    console.log(`  ${chalk.red("✗")} ${chalk.bold(tool.name)}: ${tool.installSuggestion}`);
                }
                console.log();
                if (options.fix) {
                    ...
                    for (const tool of report.missingTools) {
                        if (tool.installSuggestion.startsWith("npm install")) {
                            ...
                            await new Promise((resolve) => {
                                execFile("npm", cmd.replace("npm install ", "").split(" "), { timeout: 60000 }, (error) => {
                                    if (error) {
                            ...
                            console.log(`  ${chalk.yellow("!")} Manual install required: ${tool.installSuggestion}`);
                        }
                    }
            ...
            try {
                ...
                for (const filePath of files) {
                    ...
                    if (!(await fs.pathExists(resolved))) {
                        console.log(chalk.red("✖") + " File not found: " + chalk.bold(resolved));
                        totalErrors++;
                        continue;
                    }
                    const results = await registry.applyCorrections(resolved, undefined, true);
                    const corrected = results.filter((r) => r.corrected);
                    if (corrected.length === 0) {
                        console.log(chalk.green("✔") + " " + chalk.gray(resolved) + " — no changes");
                        continue;
                    }
                    console.log(chalk.bold("\n" + resolved));
                ...
                console.log(chalk.bold(`\n${totalCorrections} correction(s) would be applied to ${files.length - totalErrors} file(s)`));
            }
            catch (error) {
                console.error(chalk.red("✖") +
                    " Preview failed: " +
