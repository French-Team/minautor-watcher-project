import { CURRENT_YEAR, WATCHER_VERSION, } from "./types.js";
 ...
 */
export function printBanner(report) {
    ...
    console.log("");
    console.log(`╔${line}╗`);
    console.log(`║  WATCHER SERVICE v${WATCHER_VERSION} — ${CURRENT_YEAR}`.padEnd(59) + "║");
    ...
    const dateStr = system.currentDate
        ...
        .replace("T", " ")
        .substring(0, 16);
    console.log(`║  Date       : ${dateStr} (${system.timezone})`.padEnd(59) + "║");
    console.log(`║  OS         : ${system.platform} ${system.arch} (${system.osRelease})`.padEnd(59) + "║");
    console.log(`║  Host       : ${system.hostname} (${system.username})`.padEnd(59) + "║");
    console.log(`║  Node.js    : ${system.nodeVersion}`.padEnd(59) + "║");
    if (system.npmVersion) {
        console.log(`║  npm        : ${system.npmVersion}`.padEnd(59) + "║");
    }
    console.log(`║  CPU        : ${system.cpuCount} cores — ${system.cpuModel}`.padEnd(59) +
        "║");
    console.log(`║  RAM        : ${system.totalMemoryGB} GB`.padEnd(59) + "║");
    // Tools section
    console.log(`╠${line}╣`);
    console.log(`║  Tools (${availableCount}/${totalCount} available)`.padEnd(59) + "║");
    for (const tool of tools) {
        const icon = tool.available ? "✓" : "✗";
        const version = tool.version || "—";
        const toolPath = truncatePath(tool.path);
        const line = `║  ${icon} ${tool.name.padEnd(12)} ${version.padEnd(10)} ${toolPath}`;
    ...
    console.log(`║  IDE        : ${devEnv.ide.name || "not detected"}`.padEnd(59) + "║");
    console.log(`║  Shell      : ${devEnv.shell.name}`.padEnd(59) + "║");
    console.log(`║  Docker     : ${devEnv.container.isDocker ? "Yes" : "No"}`.padEnd(59) +
        "║");
    console.log(`║  WSL        : ${devEnv.container.isWSL ? `Yes (${devEnv.container.wslDistro})` : "No"}`.padEnd(59) + "║");
    console.log(`║  CI         : ${devEnv.container.isCI ? `Yes (${devEnv.container.ciProvider})` : "No"}`.padEnd(59) + "║");
    console.log(`╚${line}╝`);
    console.log("");
}
/**
 ...
 */
export function printCompactBanner(report) {
    ...
    console.log(`  ${dateStr} | ${system.platform} ${system.arch} | Node ${system.nodeVersion}`);
 ...
 */
export function getMissingToolsReport(report) {
    if (report.missingTools.length === 0) {
        return "All tools are available.";
    }
    const lines = report.missingTools.map((t) => `  ✗ ${t.name}: ${t.installSuggestion}`);
    return `Missing tools:\n${lines.join("\n")}`;
}
/**
 * Get solutions report as string array
 */
export function getSolutionsReport(report) {
    return report.suggestions;
}
//# sourceMappingURL=env-reporter.js.map