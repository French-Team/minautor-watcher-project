...
async function detectLanguage(projectDir) {
    ...
    const hasJsOnly = !hasTsDep && pkg.includes('"@babel/plugin-transform-typescript"') === false;
    if (hasTsDep) {
    ...
    try {
        ...
        for (const entry of entries) {
            if (entry.isDirectory() && !excludeDirs.includes(entry.name)) {
                const sub = await hasFilesWithExtension(path.join(dir, entry.name), ext);
                if (sub)
                    return true;
            }
 ...
 */
async function detectTestFramework(projectDir) {
    const pkg = await readSafe(path.join(projectDir, "package.json"));
    if (!pkg)
        return undefined;
 ...
 */
async function detectArchitecture(projectDir) {
    // Monorepo indicators
    if (await exists(path.join(projectDir, "lerna.json")))
        return "monorepo";
    ...
    if (pkg) {
        try {
            ...
            if (parsed.main || parsed.exports) {
                ...
                if (!(await exists(path.join(projectDir, "src"))))
                    return "library";
            }
        }
        catch {
 ...
 */
async function detectConventions(projectDir) {
    const conventions = {
        indentStyle: "spaces",
        indentSize: 2,
        lineEnding: "lf",
    ...
    if (prettierConfig) {
        try {
            ...
            if (parsed.tabWidth)
                conventions.indentSize = parsed.tabWidth;
            if (parsed.useTabs)
                conventions.indentStyle = "tabs";
        }
        catch {
...
export async function analyzeProject(projectDir) {
    ...
    const [language, packageManager, testFramework, architecture, conventions, hasTypeScript, hasESLint, hasPrettier, hasTests, hasConsignmentFiles, srcDir, testDir, configDir, packageJsonExists, tsconfigExists,] = await Promise.all([
        ...
        exists(path.join(projectDir, ".eslintrc.cjs")).then((v) => v ||
            exists(path.join(projectDir, ".eslintrc.js")).then((v2) => v2 ||
                exists(path.join(projectDir, ".eslintrc.json")).then((v3) => v3))),
        exists(path.join(projectDir, ".prettierrc")).then((v) => v ||
            exists(path.join(projectDir, ".prettierrc.json")).then((v2) => v2 ||
                exists(path.join(projectDir, ".prettierrc.js")).then((v3) => v3))),
        exists(path.join(projectDir, "tests")).then((v) => v ||
            exists(path.join(projectDir, "test")).then((v2) => v2 || exists(path.join(projectDir, "__tests__")).then((v3) => v3))),
        detectConsignmentFiles(projectDir),
        exists(path.join(projectDir, "src")),
        exists(path.join(projectDir, "tests")).then((v) => v || exists(path.join(projectDir, "test"))),
    ...
    if (pkg) {
        if (pkg.includes('"react"'))
            ...
            framework = "vue";
        else if (pkg.includes('"angular"'))
            framework = "angular";
        else if (pkg.includes('"svelte"'))
            framework = "svelte";
        else if (pkg.includes('"express"'))
            framework = "express";
        else if (pkg.includes('"fastify"'))
            framework = "fastify";
        else if (pkg.includes('"next"'))
            framework = "next";
        else if (pkg.includes('"nuxt"'))
            framework = "nuxt";
    }
    return {
        name: dirName,
 ...
 */
async function detectConsignmentFiles(projectDir) {
    const files = [
        "CLAUDE.md",
        "AGENTS.md",
        ".cursorrules",
...
export function formatAnalysis(analysis) {
    const lines = [
        ...
        `Tests: ${analysis.hasTests ? "yes" : "no"}${analysis.testFramework ? ` (${analysis.testFramework})` : ""}`,
        `Architecture: ${analysis.architecture || "unknown"}`,
        `Framework: ${analysis.framework || "none detected"}`,
        `Consignment files: ${analysis.consignmentFiles.length > 0
            ? analysis.consignmentFiles.join(", ")
            : "none"}`,
        `Conventions: ${analysis.conventions.indentSize}${analysis.conventions.indentStyle === "tabs" ? "tab" : "spaces"}, ${analysis.conventions.quotes} quotes, ${analysis.conventions.semicolons ? "" : "no "}semicolons`,
    ];
    return lines.join("\n");
}
