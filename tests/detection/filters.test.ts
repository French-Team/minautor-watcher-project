...
import os from 'os';
import { FileFilter, FilterPresets, createFileFilter, FileEvent } from '../../src/detection/filters.js';
...
describe('FileFilter', () => {
  ...
  describe('filter by exclude patterns', () => {
    it('should block paths matching exclude patterns', async () => {
      ...
      const result = await filter.apply(createEvent('/project/node_modules/pkg/index.js', 'node_modules/pkg/index.js'));
      expect(result.passed).toBe(false);
  ...
  describe('filter by include patterns', () => {
    ...
    it('should block paths not matching include patterns', async () => {
      const filter = createFileFilter({ includePatterns: ['src'] });
      const result = await filter.apply(createEvent('/dist/bundle.js', 'dist/bundle.js'));
      expect(result.passed).toBe(false);
