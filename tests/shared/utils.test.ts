...
describe("Utils", () => {
  ...
  describe("pathExists", () => {
    ...
    it("should return false for non-existing files", async () => {
      expect(
        await Utils.pathExists(path.join(TEST_DIR, "nonexistent.txt"))
      ).toBe(false);
    });
  });
  ...
  describe("shouldExcludePath", () => {
    it("should exclude paths matching patterns", () => {
      ...
      expect(
        Utils.shouldExcludePath(`${cwd}/node_modules/pkg/index.js`, [
          "node_modules",
        ])
      ).toBe(true);
      expect(
        Utils.shouldExcludePath(`${cwd}/src/index.ts`, ["node_modules"])
      ).toBe(false);
    });
  });
  ...
  describe("parseFileSize", () => {
    it("should pass through numbers", () => {
      expect(Utils.parseFileSize(1024)).toBe(1024);
    });

    it("should parse bytes", () => {
      expect(Utils.parseFileSize("500B")).toBe(500);
    });

    it("should parse kilobytes", () => {
      expect(Utils.parseFileSize("1KB")).toBe(1024);
    });

    it("should parse megabytes", () => {
      expect(Utils.parseFileSize("1MB")).toBe(1048576);
    });

    it("should parse gigabytes", () => {
      expect(Utils.parseFileSize("1GB")).toBe(1073741824);
    });

    it("should parse decimal values", () => {
      expect(Utils.parseFileSize("1.5MB")).toBe(1572864);
    });
    ...
    it("should handle units with space", () => {
      expect(Utils.parseFileSize("1 MB")).toBe(1048576);
    });
