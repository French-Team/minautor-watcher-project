...
import os from 'os';
import { ConsoleNotifier, FileNotifier, NotifierRegistry, createNotifierRegistry, NotificationLevel, NotificationChannel, NotificationUtils } from '../../src/trigger/notifiers.js';
...
describe('Notifiers', () => {
  ...
  describe('NotifierRegistry', () => {
    ...
    it('should send to a specific channel', async () => {
      const registry = new NotifierRegistry();
      registry.register(NotificationChannel.CONSOLE, new ConsoleNotifier(true));
      const results = await registry.sendToChannels([NotificationChannel.CONSOLE], {
        title: 'Test',
        message: 'Test message',
        level: NotificationLevel.INFO,
        channel: NotificationChannel.CONSOLE,
