...
export class TextReplacementCorrector extends BaseCorrector {
  ...
  canCorrect(filePath: string, error?: any): boolean {
    ...
    if (this.config.conditions.filePatterns) {
      ...
      const matchesPattern = this.config.conditions.filePatterns.some(pattern =>
  ...
  private applyTextReplacement(content: string, action: any): {
    modified: boolean;
    content: string;
    details: Array<{ type: 'replace'; line: number; column: number; oldText: string; newText: string }>;
  } {
    const details: Array<{ type: 'replace'; line: number; column: number; oldText: string; newText: string }> = [];
  ...
  private applyTextInsertion(content: string, action: any): {
    modified: boolean;
    content: string;
    details: Array<{ type: 'insert'; line: number; column: number; newText: string }>;
  } {
    const details: Array<{ type: 'insert'; line: number; column: number; newText: string }> = [];
    const lines = content.split('\n');
    ...
    if (typeof targetLine === 'number' && targetLine >= 0 && targetLine <= lines.length) {
      ...
      details.push({ type: 'insert', line: targetLine, column: 0, newText: insertContent });
    }
  ...
  private applyTextDeletion(content: string, action: any): {
    modified: boolean;
    content: string;
    details: Array<{ type: 'delete'; line: number; column: number; oldText: string }>;
  } {
    const details: Array<{ type: 'delete'; line: number; column: number; oldText: string }> = [];
    const lines = content.split('\n');
    ...
    if (action.target === 'all' && action.content) {
      ...
      while ((match = regex.exec(content)) !== null) {
        ...
        details.push({ type: 'delete', line: lineNum, column: match.index, oldText: match[0] });
      }
      return {
        modified: details.length > 0,
    ...
    if (typeof action.target === 'number' && action.target >= 0 && action.target < lines.length) {
      const deletedLine = lines.splice(action.target, 1)[0];
      details.push({ type: 'delete', line: action.target, column: 0, oldText: deletedLine });
    }
...
export class CommandCorrector extends BaseCorrector {
  ...
  private async executeCommand(action: any, filePath: string): Promise<{ success: boolean; error?: Error }> {
    try {
...
export class CorrectorRegistry {
   ...
   * Apply corrections to a file
   */
  async applyCorrections(filePath: string, error?: any): Promise<CorrectionResult[]> {
