 ...
 */
const FIX_KNOWLEDGE_BASE: Record<
  string,
  { group: string; pattern: string; fixPrompt: string }
> = {
  "no-unused-vars": {
...
function extractBaseRule(rule: string): string {
...
function findGroupKey(error: PreventionError): string {
  const base = extractBaseRule(error.rule);

  if (base in FIX_KNOWLEDGE_BASE) return base;

  const normalized = base
    .replace(/^@typescript-eslint\//, "")
    .replace(/^eslint\//, "");
  if (normalized in FIX_KNOWLEDGE_BASE) return normalized;

  if (base.startsWith("TS")) {
    const tsCode = base.match(/TS\d+/)?.[0];
    if (tsCode && tsCode in FIX_KNOWLEDGE_BASE) return tsCode;
  }

  if (normalized.includes("unused")) return "no-unused-vars";
  if (normalized.includes("console")) return "no-console";
  if (normalized.includes("prefer-const")) return "prefer-const";
  if (normalized.includes("explicit-any") || normalized.includes("no-explicit-any"))
    return "no-explicit-any";
  if (normalized.includes("unsafe-member")) return "no-unsafe-member-access";
  if (normalized.includes("unsafe-call")) return "no-unsafe-call";
  if (normalized.includes("unsafe-assignment")) return "no-unsafe-assignment";
  if (normalized.includes("undef")) return "no-undef";
  if (normalized.includes("json") || normalized.includes("syntax"))
    return "json-syntax";
  if (normalized.includes("script")) return "script-failed";
  if (normalized.includes("prettier")) return "prettier-formatting";
...
function groupErrors(
  ...
  const grouped = new Map<string, PreventionError[]>();
  ...
  const groups: FixInstructionGroup[] = [];
...
function generateMarkdownReport(report: AgentFixReport): string {
  const lines: string[] = [];
...
export async function writeFixReport(
  report: AgentFixReport,
  outputDir?: string
): Promise<string> {
...
export async function cleanFixReports(projectDir: string): Promise<void> {
...
function generateInstructionsContent(): string {
  return [
 ...
 */
async function writeInstructionsIfMissing(targetDir: string): Promise<void> {
