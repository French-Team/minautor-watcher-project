import fs from "fs";
import { type DevEnvironment } from "./types.js";

/**
 * Detect IDE / Editor
 */
function detectIDE(): DevEnvironment["ide"] {
  // VS Code
 ...
 */
function detectShell(): DevEnvironment["shell"] {
  const comSpec = process.env.ComSpec || "";
  const shellEnv = process.env.SHELL || "";

  // PowerShell
  if (
    comSpec.toLowerCase().includes("powershell") ||
 ...
 */
function detectWSL(): { isWSL: boolean; distro: string | null } {
 ...
 */
export function formatDevEnvironment(env: DevEnvironment): string {
  const lines: string[] = [];
  ...
  lines.push(
    `WSL         : ${
      env.container.isWSL ? `Yes (${env.container.wslDistro})` : "No"
    }`
  );
  lines.push(
    `CI          : ${
      env.container.isCI ? `Yes (${env.container.ciProvider})` : "No"
    }`
  );
