 ...
 */
export function loadUnifiedConfig(configDir?: string): WatcherConfig {
  ...
  if (error) {
    logger.warn(
      `Config validation warnings: ${error.details
        .map((d) => d.message)
        .join(", ")}`
    );
  }
 ...
 */
function loadLegacyConfigs(config: WatcherConfig, baseDir: string): void {
 ...
 */
export function saveUnifiedConfig(
