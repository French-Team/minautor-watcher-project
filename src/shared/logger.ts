 ...
 */
export async function writeReport(data: ReportData): Promise<void> {
  const lines: string[] = [];
  const L = (s: string) => lines.push(s);
  const S = (label: string, val: string) => L(`  ${label.padEnd(18)} ${val}`);
...
function suggestion(rule: string): string {
