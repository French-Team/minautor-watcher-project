 ...
 */
const DEFAULT_RULES: AdaptiveRule[] = [
  {
    id: "no-any-type",
    name: "No any type",
    ...
    action: "enforce",
    message:
      "TypeScript detected. Never use `any` — use `unknown` and narrow types.",
    priority: 1,
  },
  {
    id: "eslint-required",
  ...
  },
  {
    ...
    action: "suggest",
    message:
      "ESLint detected but Prettier is not. Consider adding Prettier for consistent formatting.",
    priority: 3,
  },
  {
    id: "tests-required",
  ...
  },
  {
    ...
    action: "enforce",
    message:
      "Use Winston logger instead of console.log for structured logging.",
    priority: 5,
  },
  {
    id: "safe-spawn",
    ...
    action: "enforce",
    message:
      "Use safeSpawn() instead of exec() for child processes to prevent injection.",
    priority: 6,
  },
  {
    id: "sanitize-paths",
    ...
    action: "enforce",
    message:
      "Use sanitizePath() before any file path operation to prevent traversal.",
    priority: 7,
  },
  {
    ...
    action: "suggest",
    message:
      "Monorepo detected. Consider workspace-specific configs and shared packages.",
    priority: 8,
  },
  {
    ...
    action: "suggest",
    message:
      "No consignment files (CLAUDE.md, AGENTS.md). Inject them to guide AI agents.",
    priority: 9,
  },
  {
    id: "esm-modules",
 ...
 */
export function getEnforcedRules(
  analysis: ProjectAnalysis,
  rules?: AdaptiveRule[]
): RuleEvaluation[] {
  return getTriggeredRules(analysis, rules).filter(
    (e) => e.action === "enforce"
  );
}
 ...
 */
export function formatEvaluations(evaluations: RuleEvaluation[]): string {
  const triggered = evaluations.filter((e) => e.triggered);
  if (triggered.length === 0) return "No rules triggered.";
 ...
 */
export function getDefaultRules(): AdaptiveRule[] {
