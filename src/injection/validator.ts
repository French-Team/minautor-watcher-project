...
const DEFAULT_VALIDATOR_CONFIG: ConsignmentValidatorConfig = {
  enabled: true,
 ...
 */
export async function validateConsignmentFiles(
  projectDir: string,
  validatorConfig?: Partial<ConsignmentValidatorConfig>
    ...
    if (!result.isValid && config.autoFix) {
      try {
        ...
        if (errors.length === 0) {
          result.isValid = true;
          result.errors = [];
          const createdCount = injectResult.filter(
            (r) => r.action === "created"
          ).length;
          result.warnings.push({
 ...
 */
export function formatConsignmentResult(result: ValidationResult): string {
  const lines: string[] = [];
